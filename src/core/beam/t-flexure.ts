...
export function calculateBeamTFlexure(input: BeamTFlexureInput): CalculationResult {
  ...
  steps.push({
    name: '计算最小配筋面积',
    description: 'As,min = max(0.2%, 45ft/fy%) · [b·h + (bf-b)·hf]',
    formula: 'A_s,min = ρ_min · [b · h + (b_f - b) · h_f]',
    substitutedFormula: `A_s,min = ${rhoMinPercent.toFixed(3)}% × [${input.b} × ${input.h} + (${input.bf} - ${input.b}) × ${input.hf}] / 100`,
